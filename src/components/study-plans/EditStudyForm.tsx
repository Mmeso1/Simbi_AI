...
import { Study } from "@/types/studyPlan";
...
export default function EditStudyForm({
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    try {
      await updateStudy(studyToEdit.id, data);
      toast.success("Study plan updated successfully!");
      handleToggleEditGenerateStudyPlan();
      router.refresh(); // Optional: to re-fetch updated data if needed
    } catch (error) {
      console.error("Failed to update study plan:", error);
      toast.error("Failed to update study plan. Please try again.");
    } finally {
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex justify-between items-center mt-6 md:flex-row flex-col-reverse">
            ...
            <div className="flex items-center md:w-1/2 w-full">
              ...
              >
                {isSubmitting ? "Updating..." : "Update Study Plan"}
              </button>
            </div>
          </div>
        </form>
